...
ruby -EUTF-8:UTF-8 "$(command -v bundle)" exec jekyll build -d "${site_no_compat}" >/dev/null
...
ruby -EUTF-8:UTF-8 "$(command -v bundle)" exec jekyll build --config "_config.yml,${override_file}" -d "${site_with_compat}" >/dev/null
